...
async def health_tips_endpoint(category: str = Query("General", description="Wellness category tag")):
    """
    Returns 3 preventative daily wellness tips for the selected category.
    """
    if not GROQ_API_KEY:
        tips_list = generate_mock_tips(category)
    ...
    try:
        ...
        tips_data = await generate_structured_response(
            ...
            system_instruction=SYSTEM_TIPS_PROMPT
        )
        return tips_data
